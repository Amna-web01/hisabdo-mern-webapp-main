    ...
    <nav className="bg-slate-900 text-white border-b border-slate-800 sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 flex-nowrap">
          ...
          <Link 
            href="/" 
            className="flex items-center space-x-2 font-bold text-lg sm:text-xl text-emerald-400 whitespace-nowrap shrink-0"
          >
          ...
          <button 
            ...
            className="md:hidden text-slate-300 hover:text-white p-1 shrink-0"
      ...
      {isOpen && (
        <div className="md:hidden bg-slate-800 border-b border-slate-700 px-4 pt-2 pb-4 space-y-2 text-sm font-medium text-slate-200">
          <Link href="/" onClick={() => setIsOpen(false)} className="block py-2 hover:text-emerald-400">Home</Link>
          <Link href="/about" onClick={() => setIsOpen(false)} className="block py-2 hover:text-emerald-400">About</Link>
          <Link href="/features" onClick={() => setIsOpen(false)} className="block py-2 hover:text-emerald-400">Features</Link>
          <Link href="/pricing" onClick={() => setIsOpen(false)} className="block py-2 hover:text-emerald-400">Pricing</Link>
          <Link href="/contact" onClick={() => setIsOpen(false)} className="block py-2 hover:text-emerald-400">Contact</Link>
          <Link href="/dashboard" onClick={() => setIsOpen(false)} className="block mt-2 bg-emerald-500 text-slate-950 text-center py-2 rounded-lg font-bold">
            Open Dashboard
          </Link>
        </div>
      )}
